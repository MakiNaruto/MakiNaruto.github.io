---
layout: post
title: "Stanford / Winter 2020 CS224n 课程学习笔记01-02"
subtitle: "Assignment"
author: "MakiNaruto"
header-img: "img/WestWorld.jpg"
header-mask: 0.4
mathjax: true
tags:
  - Assignment
...
![](https://upload-images.jianshu.io/upload_images/17373898-1d2b2881f1c1ea05.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)
...
![](https://upload-images.jianshu.io/upload_images/17373898-99b440b51b8b6f1a.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)
计算完毕后，会得到一个和词汇表长度一样的概率输出：即$$\hat y = [0.92，0.03，0.01，0.06，0.07，0.25]。$$
#### 疑问二：词向量是怎么训练出来的？
首先要用到交叉熵损失函数 （cross-entropy loss） ：

$$
 loss_{cross-entropy} = -\sum_{w \in V o c a b} y_{w} \log \left(\hat{y}_{w}\right)
$$

即预测值与真实值（目标值）的差异，即$y = [1，0，0，0，0，0]，\hat y = [0.92，0.03，0.01，0.06，0.07，0.25]$，差异性越大，交叉熵损失越大。所以为了让预测更准确，我们要降低交叉熵损失
...
注：在开始时生成两个同样大小的矩阵$U，V$。$U$初始为0矩阵，$V$则进行随机初始化，两个矩阵大小都是Vocab * dim。但是，进行更新时，$ \frac{\partial J }{\partial v_{c}}$ 只在$V$内更新，$ \frac{\partial J }{\partial w_{u}}$只在$U$内更新。

#### 为了更直观的理解，下面是我做的动画演示。
![softmax_vc的更新](https://upload-images.jianshu.io/upload_images/17373898-fc71a53a98165521.gif?imageMogr2/auto-orient/strip)
![Softmax_U的更新](https://upload-images.jianshu.io/upload_images/17373898-468fc147ebf57a1a.gif?imageMogr2/auto-orient/strip)
...
#### 动画演示。
![sigmoid_U的更新](https://upload-images.jianshu.io/upload_images/17373898-4afdaa380863bf8d.gif?imageMogr2/auto-orient/strip)
![sigmoid_Vc的更新](https://upload-images.jianshu.io/upload_images/17373898-77edb20adc20c6c3.gif?imageMogr2/auto-orient/strip)
